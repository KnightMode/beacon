...
5. **Plan when needed** — the first hybrid pass is the default fast path. With
   ...
   evidence is weak or the question explicitly asks for tracing, references,
   cross-repo impact, or breaking-change analysis. `always` forces deeper
   planner loops; `off` disables them.
