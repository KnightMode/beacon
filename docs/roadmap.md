...
4. **Zoekt + SCIP foundation** — in progress: GitHub Actions remains the heavy
   index-generation runner; Zoekt is the exact-search substrate served through a
   Cloudflare Container; normalized SCIP facts sit beside `code_edges` for
   definitions/references. Next: backfill real customer repos and tune evals.
5. **Feedback loop** — capture :+1:/:-1: on answers into D1 to grow the eval set
   from real usage and surface bad-answer patterns.
6. **Structural tenant isolation** — move from tenant-scoped rows in shared D1
   to per-tenant D1 databases plus Vectorize namespaces before broad SaaS
   rollout.
7. **Cross-encoder reranking** — a real reranker over the top ~50 candidates
   before context packing; the highest ROI-per-line change left in retrieval.
8. **Frontier answer model** — route the final answer (and the agent planner) to
   a stronger model; retrieval quality is increasingly ahead of the 8–30B-class
   models summarizing it.
9. **Permission-aware retrieval** — per-user GitHub OAuth + permission sync so
   each Slack user only sees repos they can access (schema already supports it).
10. **Index the conversation, not just the code** — PR descriptions, review
